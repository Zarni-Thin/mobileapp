...
function delLocalStorage() {
    ...
    del.addEventListener("click",
        function (e) {
            ...
            if (w_cnt >= 1) {
                let w_msg = "LocalStorage から 選択されている " + w_cnt + "件 を削除(delete)しますか？";
                Swal.fire({
                    title: "Memo app"
                    , html: w_msg
                    , type: "question"
                    ...
                    if (result.value === true) {
                        ...
                        viewStorage();
                        let w_msg = "LocalStorage から " + w_cnt + "件 を削除 (delete)しました。";
                        Swal.fire({
                            title: "Memo app"
                            , html: w_msg
                            , type: "success"
    ...
    table1.addEventListener("click", (e) => {  // eはイベントの対象要素…変数なので、名前はなんでもよい。
        if (e.target.classList.contains("trash") === true) {
            ...
            let w_delete = `LocalStorage から\n[${key} ${value}] を削除 (delete)しますか？`;
            Swal.fire({
                title: "Memo app"
                , html: w_delete
                , type: "question"
                ...
                if (result.value === true) {
                    ...
                    viewStorage();
                    let w_msg = `LocalStorage から ${key} ${value}件 を削除 (delete)しました。`;
                    Swal.fire({
                        title: "Memo app"
                        , html: w_msg
                        , type: "success"
